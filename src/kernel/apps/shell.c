...
static int tryExecBuiltin(Context *ctxt, const char *app, int argc, char **argv)
{
    ...
    // scripting
    if (strcmp(app, "script") == 0) 
        return enter(ctxt, scriptMain, argc, argv);
    
    // just to check time (only in hours)
    if (strcmp(app, "printtime") == 0)
        return enter(ctxt, printtime, argc, argv);
