// shamelessly stolen and modified from https://codeberg.org/matthilde/mbasic/
#include <stdio.h>
#include <stdlib.h>
...
// stub implementation
unsigned long timestub(unsigned long *t) {return 0;};
char varnames[64][8];
int  varcontent[64];
void initvars() { srand(timestub(NULL)); for(int i=0; i<64; ++i) varnames[i][0] = 0; }
...
int cinput(int ln, char* s) {
	char vs[16], vn[16]; scpy(vn, _strtok(s));
...
int cgoto(int ln, char* s) {
	char *tok = _strtok(s); if (!tok) berror(ln, "INVALID GOTO");
	return emath(tok)-1;
}
int cgosub(int ln, char* s) {
	char *tok = _strtok(s); if (!tok) berror(ln, "INVALID GOSUB");
